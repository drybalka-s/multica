...
// provider on Linux/macOS gets the same safe single-quoted HEREDOC template.
// Inline `--content "..."` lets the shell expand `$()`, variables, quotes, and
// code snippets before multica receives the body.
//
// Not parallel: mutates the package-level runtimeGOOS.
			...
			t.Run(name, func(t *testing.T) {
				...
				for _, banned := range []string{
					...
					if strings.Contains(got, banned) {
						t.Errorf("%s reply instructions still offers unsafe form: %q\n---\n%s", name, banned, got)
					}
				}
			})
		}
