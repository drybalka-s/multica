...
func BuildPrompt(task Task, provider string) string {
	...
	fmt.Fprintf(&b, "For comment history, follow the rule in your runtime workflow file (assignment-triggered tasks treat the read as mandatory). `multica issue comment list %s --all --output json` reads the full history when mandatory. Default comment listing is capped to `--recent 10`; on long-running issues use `--recent 20 --summary --output json`, then page older threads via the stderr `Next thread cursor: ...` line and the matching `--before` / `--before-id`. `--since <RFC3339>` is still available for incremental polling and may combine with `--recent`.\n", task.IssueID)
	return b.String()
}
